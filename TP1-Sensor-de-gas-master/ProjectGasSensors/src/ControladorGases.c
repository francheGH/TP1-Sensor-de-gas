...
void fmodo_P(int mq, int *total[3]) {//cambia los valores de total
	...
	lcdSendStringRaw(str2, str);/*prueba sensor*/
	if (mq == 4) {/*prueba teclado*/
		if (keypadRead(&keypad, &tecla)) {
			entrada[0] = tecla;
			entrada[1] = entrada[0];
			entrada[2] = entrada[1];
			entrada[3] = entrada[2];
			int *total[0] = entrada[0] * 1000 + entrada[1] * 100
			...
			if (keypadRead(&keypad, &tecla)) {
				...
				entrada[2] = entrada[1];
				entrada[3] = entrada[2];
				int *total[1] = entrada[0] * 1000 + entrada[1] * 100
						+ entrada[2] * 10 + entrada[3];
			} else if (mq == 7) {
				...
				if (keypadRead(&keypad, &tecla)) {
					...
					entrada[2] = entrada[1];
					entrada[3] = entrada[2];
					int *total[2] = entrada[0] * 1000 + entrada[1] * 100
							+ entrada[2] * 10 + entrada[3];
				}
			}
